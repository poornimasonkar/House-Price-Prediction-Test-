...
import os
from PIL import Image  # ✅ This is required

if os.path.exists("feature_importance.png"):
    image = Image.open("feature_importance.png")
    st.image(image, caption="Which features matter most", use_column_width=True)
else:
...
X = df.drop("MEDV", axis=1)   # MEDV is usually the target column in Boston dataset
y = df["MEDV"]
...
# X_test, y_test must be from your notebook or reloaded
# Or recreate using train.csv
...
with st.expander("ℹ️ About this App"):
    st.markdown("""
    - 🎯 This app predicts Boston housing prices using a Linear Regression , Random Forest & XG Boost Regressor model.
    - 📚 Trained on 13 features including crime rate, number of rooms, tax rate, etc.
    - 🧠 Model: Linear Regression ,Random Forest Regressor , XG Boost Regressor
